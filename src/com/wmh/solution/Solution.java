...
class Solution {
	...
	private class Node implements Comparable<Node> {
		...
		public Node(char data, int size) {
			super();
			this.data = data;
			this.size = size;
		}
	...
	public String frequencySort(String s) {
		...
		for(char ch:map.keySet()) {
			heap.add(new Node(ch,map.get(ch)));
		}
		StringBuilder result = new StringBuilder(); 
		while(!heap.isEmpty()) {
			Node extractMax = heap.extractMax();
			for(int i=0;i<extractMax.size;i++) {
				result.append(extractMax.data);
			}
	...
	public static void main(String[] args) {
		...
		String map = s.frequencySort("adgfdfgdwessdf");
